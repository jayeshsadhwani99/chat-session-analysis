...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[logging.FileHandler("pipeline_output.txt"), logging.StreamHandler()],
    )
    ...
    try:
        ...
        for i, sess_chunk in enumerate(reader, start=1):
            ...
            if len(user_df) == 0:
                logger.info(
                    f"--- Chunk {i}: sessions={len(sess_chunk)}, user_msgs=0, sampled=0"
                )
                continue
            ...
            logger.info(
                f"--- Chunk {i}: sessions={len(sess_chunk)}, user_msgs={len(user_df)}, sampled={len(sampled)}"
            )

            # Include session metadata in the sampled output
            sampled_chunks.append(
                sampled[["session_id", "query", "session_length", "num_user_messages"]]
            )
        ...
        summary = {
            ...
            "intent_breakdown": {
                str(k): int(v) for k, v in intent_counts.to_dict().items()
            },
            "funnel_breakdown": {
                str(k): int(v) for k, v in funnel_counts.to_dict().items()
            },
            "brand_breakdown": {
                k: int(v) for k, v in brand_counts.head(20).to_dict().items()
            },
            "support_count": int(support_count),
            "support_percentage": round(support_pct, 2),
            "session_metrics": {
                "avg_session_length": float(sample_df["session_length"].mean()),
                "avg_user_messages_per_session": float(
                    sample_df["num_user_messages"].mean()
                ),
                "median_session_length": float(sample_df["session_length"].median()),
                "median_user_messages_per_session": float(
                    sample_df["num_user_messages"].median()
                ),
            },
        }
